class ScheduledProcessingMailer < ApplicationMailer
  def check_notice_mail
    ...
    User.all.each do |user|
      temp_users << user if (user.passive_notifications.where(visited_id: user.id, checked: false).where("created_at < ?", 3.days.ago.beginning_of_day).present?)
    end
  ...
  def high_pv_mail
    ...
    Post.all.each do |post|
      temp_users << post.user if (post.impressionist_count >=30) && !(temp_users.include?(post.user))
    end
